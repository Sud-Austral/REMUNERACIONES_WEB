...
async function obtenerRespuesta(pregunta) {
    ...
    // 🔹 Paso 1: generar un resumen agregado de los datos
    const resumen = {};

    for (const d of data["datos"]) {
        const key = `${d.organismo}-${d.año}-${d.tipo_contrato}-${d.calificacion}`;
        ...
        resumen[key].totalPersonas += d.personas;
        resumen[key].totalRemuneracion += d.remuneracion * d.personas; // remuneración ponderada
        resumen[key].count += d.personas;
    }

    // 🔹 Paso 2: convertir el resumen a texto
    const resumenTexto = Object.values(resumen).map(r =>
        `- Org: ${r.organismo}, Año: ${r.año}, Contrato: ${r.tipo_contrato}, Calificación: ${r.calificacion}, Personas: ${r.totalPersonas}, RemProm: $${Math.round(r.totalRemuneracion / r.count).toLocaleString()}`
    ).join('\n');

    // 🔹 Paso 3: armar el prompt base
    const promptBase = `Eres un asistente experto en análisis de datos de empleo público en Chile.

    Datos agregados (mantienen exactitud):
    ${resumenTexto}

    Instrucciones:
        1. Responde de manera clara y concisa.
        2. No detalles el proceso para llegar al resultado.
